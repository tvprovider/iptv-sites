 ...
 */
export function writeSnapshot(args: WriteSnapshotArgs): string | null {
  try {
    ensureDir();
    ...
    process.stderr.write(
      `[namecheap-mcp] snapshot write failed for ${args.domain}: ${err instanceof Error ? err.message : String(err)}\n`
    );
    return null;
  }
}
 ...
 */
export function listSnapshots(domain: string): SnapshotInfo[] {
  try {
 ...
 */
export function readSnapshot(snapshotPath: string): DnsSnapshot {
  const raw = fs.readFileSync(snapshotPath, 'utf-8');
  const parsed = JSON.parse(raw) as DnsSnapshot;
  if (parsed.version !== SNAPSHOT_VERSION) {
    throw new Error(
      `Snapshot version ${parsed.version} not supported (expected ${SNAPSHOT_VERSION}): ${snapshotPath}`
    );
  }
 ...
 */
export function pruneSnapshots(domain: string, keep: number = DEFAULT_RETENTION): void {

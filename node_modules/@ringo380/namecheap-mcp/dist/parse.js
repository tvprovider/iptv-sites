 ...
 */
export function parseGetHostsResponse(result) {
    if (!result || typeof result !== 'object') {
        throw new Error('getHosts returned a non-object response; refusing to proceed to avoid zone wipe.');
    }
    const r = result['DomainDNSGetHostsResult'];
    if (!r || typeof r !== 'object') {
        throw new Error('getHosts response missing DomainDNSGetHostsResult envelope; refusing to proceed to avoid zone wipe. ' +
            'Raw response keys: ' + Object.keys(result).join(', '));
    }
    const envelope = r;
    ...
    const raw = Array.isArray(rawHostValue)
        ? rawHostValue
        : rawHostValue
            ? [rawHostValue]
            : [];
    const hosts = raw.map((h) => {
        ...
        if (hostName === undefined || recordType === undefined || address === undefined) {
            throw new Error('getHosts response contained a Host element missing required @_Name/@_Type/@_Address attribute; refusing to proceed to avoid zone wipe.');
        }
        return {
            hostName,

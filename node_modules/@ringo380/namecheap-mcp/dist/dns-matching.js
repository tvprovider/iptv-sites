...
export function recordMatches(existing, target) {
    if (existing.hostName !== target.hostName)
        return false;
    if (existing.recordType !== target.recordType)
        return false;
    if (MULTI_VALUE_TYPES.has(target.recordType) && target.address !== undefined) {
        return existing.address === target.address;
    }
    return true;
}
export { MULTI_VALUE_TYPES };
//# sourceMappingURL=dns-matching.js.map
...
function logoMark({ height = 32, onDark = false } = {}) {
  const width = Math.round(height * (180 / 32));
  const textFill = onDark ? '#ffffff' : '#1c1310';
  return `<svg width="${width}" height="${height}" viewBox="0 0 180 32" role="img" aria-label="IPTV Latino — IPTV subscription logo"><rect width="32" height="32" rx="7" fill="#1c1006"/><rect x="6" y="7" width="20" height="15" rx="3" fill="#d97706"/><rect x="8.2" y="9.2" width="15.6" height="10" rx="1.8" fill="#1c1006"/><path d="M13.5 11 L13.5 17.4 L19 14.2 Z" fill="#ffffff"/><rect x="14" y="22" width="4" height="3" fill="#d97706"/><rect x="10" y="25" width="12" height="2" rx="1" fill="#d97706"/><text x="42" y="23" font-family="Inter, Arial, sans-serif" font-size="16" font-weight="600" fill="${textFill}">IPTV Latino</text></svg>`;
}
    ...
    <div class="container">
      <div class="footer-grid">
        <div>
          ...
          <p style="max-width:32ch;">Fútbol, novelas, and the full English catalog on one login, with transparent pricing and real support.</p>
        </div>
...
<svg viewBox="0 0 560 420" role="img" aria-label="${esc(label)}">
  ...
  <rect x="60" y="60" width="220" height="220" rx="12" fill="none" stroke="#fde68a" stroke-width="2"/>
  <rect x="110" y="110" width="220" height="220" rx="12" fill="#ffffff" stroke="#fcd34d" stroke-width="2"/>
  <rect x="160" y="160" width="220" height="220" rx="12" fill="#fef3c7" opacity="0.7"/>
  <circle cx="270" cy="270" r="46" fill="#d97706"/>
  ${iconInner}
</svg>`;
}

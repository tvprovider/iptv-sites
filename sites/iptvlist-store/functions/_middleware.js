// Canonical host + scheme enforcement: www.iptvlist.store and any plain-HTTP
// request 301-redirect to https://iptvlist.store, avoiding duplicate content
// across the www/non-www and http/https variants of the site.
export async function onRequest(context) {
  const url = new URL(context.request.url);
  let changed = false;

  if (url.hostname === 'www.iptvlist.store') {
    url.hostname = 'iptvlist.store';
    changed = true;
  }

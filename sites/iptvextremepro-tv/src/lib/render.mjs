...
function logoMark({ height = 32, onDark = false } = {}) {
  const width = Math.round(height * (206 / 32));
  const textFill = onDark ? '#ffffff' : '#14100f';
  return `<svg width="${width}" height="${height}" viewBox="0 0 206 32" role="img" aria-label="IPTV Xtream Pro — IPTV subscription logo"><rect width="32" height="32" rx="7" fill="#1a0f08"/><rect x="6" y="7" width="20" height="15" rx="3" fill="#ea580c"/><rect x="8.2" y="9.2" width="15.6" height="10" rx="1.8" fill="#1a0f08"/><path d="M13.5 11 L13.5 17.4 L19 14.2 Z" fill="#ffffff"/><rect x="14" y="22" width="4" height="3" fill="#ea580c"/><rect x="10" y="25" width="12" height="2" rx="1" fill="#ea580c"/><text x="42" y="23" font-family="Inter, Arial, sans-serif" font-size="16" font-weight="600" fill="${textFill}">IPTV Xtream Pro</text></svg>`;
}

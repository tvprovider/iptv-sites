...
function logoMark({ height = 32, onDark = false } = {}) {
  const width = Math.round(height * (192 / 32));
  const textFill = onDark ? '#ffffff' : '#3f0f1e';
  return `<svg width="${width}" height="${height}" viewBox="0 0 192 32" role="img" aria-label="IPTV Poland — Ekstraklasa and Polish TV IPTV subscription logo"><rect width="32" height="32" rx="7" fill="#2c0810"/><rect x="6" y="7" width="20" height="15" rx="3" fill="#9f1239"/><rect x="8.2" y="9.2" width="15.6" height="10" rx="1.8" fill="#2c0810"/><path d="M13.5 11 L13.5 17.4 L19 14.2 Z" fill="#ffffff"/><rect x="14" y="22" width="4" height="3" fill="#9f1239"/><rect x="10" y="25" width="12" height="2" rx="1" fill="#9f1239"/><text x="42" y="23" font-family="Inter, Arial, sans-serif" font-size="16" font-weight="600" fill="${textFill}">IPTV Poland</text></svg>`;
}
    ...
    <div class="container">
      <div class="footer-grid">
        <div>
          ...
          <p style="max-width:32ch;">Ekstraklasa, Polish news, and Polish TV, streamed wherever you actually live.</p>
        </div>
...
<svg viewBox="0 0 560 420" role="img" aria-label="${esc(label)}">
  ...
  <rect x="60" y="60" width="220" height="220" rx="12" fill="none" stroke="#fecdd3" stroke-width="2"/>
  <rect x="110" y="110" width="220" height="220" rx="12" fill="#ffffff" stroke="#fda4af" stroke-width="2"/>
  <rect x="160" y="160" width="220" height="220" rx="12" fill="#ffe4e6" opacity="0.7"/>
  <circle cx="270" cy="270" r="46" fill="#9f1239"/>
  ${iconInner}
</svg>`;
}
